...
mirror_dict = {
    "AR": {"country_code": "AR", "country_name": "Argentina"},
...
def get_mirror_list() -> list[str]:
    return list(map(lambda c: c["country_name"], mirror_dict.values()))


def get_mirror_country_by_code(country_code="US") -> str:
    if country_code not in mirror_dict.keys():
        country_code = "US"
    return mirror_dict[country_code]["country_name"]
